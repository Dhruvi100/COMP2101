...
# Download and extract the tarfile only if it doesn't exist in the current directory
if [ ! -f pics.tgz ]; then
    wget -q https://zonzorp.net/pics.tgz -O pics.tgz || (echo "Failed to download the tarfile" && exit 1)
fi

# Extract the tarfile into the pics directory
